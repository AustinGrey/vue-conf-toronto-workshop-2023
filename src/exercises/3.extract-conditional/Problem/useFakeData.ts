...
import type { Message, User, Notification } from './types'
...
const initialize = () => {
  ...
  onlineUsers.value = [
    ...
    { id: 3, name: 'Cedric', isOnline: false }
  ]

  // Find first message from Bob
  const bob = [...messages.value].reverse().find((message) => message.from === 'Bob')
  if (bob) {
    const firstNotif = new Date(`12/12/12 ${bob.time}`)
    notifications.value = [
      {
        id: 1,
        from: 'Bob',
        content: 'Bob is now online',
        time: new Date(firstNotif.getTime() - 10 * 1000).toLocaleTimeString()
      }
    ]
  }
  ...
  watch(live, () => {
    if (live.length === 0) {
      ...
      setTimeout(() => {
        // Bob logs off
        onlineUsers.value[1].isOnline = false
        notifications.value.push({
          id: 3,
          from: 'Bob',
          content: 'Bob is now offline',
          time: new Date().toLocaleTimeString()
        })
      }, 4_000)
    }
